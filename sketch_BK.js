...
function setup() {
	...
	for (let i = 0; i <= _gridSizes.length; i++) {
		// create set of start coords starting at centre and moving 1 square up and to the left on each tick
		const gridStartX = _centerX - _sqSize * (i + 1);
		const gridStartY = _centerY - _sqSize * (i + 1);
	...
	// confirm centre point
	// stroke('white');
	// square(_centerX, _centerY, 20);
